...
Q = eye(4)*5; %previous Q doesnt seem to work
R = [ 1 ; 1 ];
...
for i=14:size(rthetacentroidData,2)
    if(size(xytransposecentroidData{1,i},2)>1)
        ...
        radiuserror(i) = smallestCentroid(1,1);
        thetaerror(i) = smallestCentroid(2,1);
    end
    if(~isempty(xytransposecentroidData{1,i}))
        radiuserror(i) = rthetacentroidData{1,i}(1,1);%to store just the radius to see error
        thetaerror(i) = rthetacentroidData{1,i}(2,1);
    end
end
...
for i=1:size(rthetacentroidData,2)
    ...
    if(~(isempty(rthetacentroidData{1,i})))
        [xpred, Ppred] = predict(x, P, A, Q);%state prediction
        ...
        xpredtracked(:,i) = xpred;
        delT = (50*10^-3); %resets deltaT once observation is received
     %   F = expm(A*delT); %Resets state transition matrix
     A = [ 1 delT 0 0 ; 0 1 0 0 ; 0 0 1 delT ; 0 0 0 1]; %System Matrix
        foundOne= true;
    else
        if(foundOne == true)
            delT = delT + (50*10^-3); %Everytime there is a missing frame, increments by frame
            A = [ 1 delT 0 0 ; 0 1 0 0 ; 0 0 1 delT ; 0 0 0 1] %System Matrix
        end
    end
