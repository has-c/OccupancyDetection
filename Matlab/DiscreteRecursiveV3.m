...
for i = 1:min(size(rthetacentroid,2),size(centroid,2))
    val = min(minDist(~ismember(minDist,0)));
    [minRowOrg,minColOrg] = find(val==minDistOrg);
    [minRow,minCol] = find(val==minDist);
    centroid(:,minRowOrg).currentFrame = rthetacentroid(:,minColOrg);
    minDist(minRow,:) = [] ;%remove to find local minimum
    minDist(:,minCol)= [];
    rthetacentroidCurrent(:,minCol) = []; %removes from remaining, so that remaining can be added
end
index = 1;

if(~isempty(rthetacentroidCurrent))
    for i = size(centroid,2)+1: size(rthetacentroid,2)
...
%     for k = 1 : length(centroid(2).xTrack)
%     %figure(2)
%         polarscatter(centroid(2).xTrack(3,k),centroid(2).xTrack(1,k));
%       hold on;
